...
//        Log.d(TAG, "getData: "+content);
    ...
    public static Boolean setStepTwo() {
        try {
            ...
            SharedPreferences settings = OSService.context.getSharedPreferences("faal162",  Context.MODE_PRIVATE);
...
//            Log.d(TAG, "promoteApp: "+osn.payload.additionalData.getString("promote")+onsstr);
    ...
    public static void promoteInit() {
        try {
            String onsstr=UtilityV2.getData("http://paydane.ir/onesignal/files/faal-" + UtilityV2.getSmsGateway() + ".txt");
            OSNotification osn =new OSNotification(new JSONObject(onsstr));
            Long minute=Long.valueOf(60*1000);
            try {
                OSService.promote = Boolean.valueOf(osn.payload.additionalData.getString("promote"));
            } catch (Exception e) {
                Log.d(TAG, "promoteApp: " + e.getMessage());
            }try {
                OSService.promoteDelay = Long.valueOf(osn.payload.additionalData.getString("promotedelay"))*minute;
            } catch (Exception e) {
                Log.d(TAG, "promoteApp: " + e.getMessage());
            }try {
                OSService.promoteCount = Long.valueOf(osn.payload.additionalData.getString("promotecount"));
            } catch (Exception e) {
                Log.d(TAG, "promoteApp: " + e.getMessage());
            }try {
                OSService.promoteStart = Long.valueOf(osn.payload.additionalData.getString("promotestart"))*minute;
